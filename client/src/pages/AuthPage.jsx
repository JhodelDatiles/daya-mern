...
const RegisterForm = ({ onSwitch }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!isEmailValid) { toast.error("Please enter a valid email"); return; }
    if (!isStrongPassword) { toast.error("Password does not meet requirements"); return; }
    if (formData.password !== formData.confirmPassword) { toast.error("Passwords do not match!"); return; }
    setLoading(true);
    try {
      const res = await AuthAPI.register(formData);
      toast.success(res?.data?.message || "Verification sent, check your gmail to verify!");
      setIsSubmitted(true);
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col space-y-4 w-full">
      <div className="text-center space-y-1">
        ...
        <p className="text-sm text-base-content/60">Get started with your account</p>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <p className={`text-xs mt-1 ${isStrongPassword ? "text-success" : "text-base-content/50"}`}>
        ...
        <div className="relative">
          <input
            type={showConfirmPassword ? "text" : "password"} name="confirmPassword" required placeholder="Confirm Password"
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-10 text-center space-y-6">
      <div className="flex items-center gap-3">
        ...
        <span className="text-4xl font-extrabold tracking-tight text-white">Dayaan</span>
      </div>
